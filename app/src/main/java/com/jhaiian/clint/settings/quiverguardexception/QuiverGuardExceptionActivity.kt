...
// Manages the per-site Quiver Guard exception list. Sites added here bypass all
// Quiver Guard filtering (both network blocking and cosmetic injection) so the
// user can opt specific domains out without disabling the feature globally.
//
// The exception list is stored using the generic SitePermissionDatabase with
// TYPE_QUIVER_GUARD_EXCEPTION. The engine reads these entries via
// SitePermissionManager in MainQuiverGuardDelegate before each page load.
class QuiverGuardExceptionActivity : ClintActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_quiver_guard_exception)

        val toolbar = findViewById<View>(R.id.quiver_guard_exception_toolbar)
        ...
        // Offset both FABs above the navigation bar so they don't overlap system UI.
    ...
    override fun onResume() {
        super.onResume()
        // Refresh in onResume so the list reflects exceptions added or removed from
        // site-settings dialogs elsewhere in the app without requiring a full restart.
        refreshList()
    }
    ...
    private fun setupRecycler() {
        adapter = SitePermissionAdapter(
            ...
            // The state label uses the primary colour to communicate that the site
            // is actively excepted from filtering.
            stateToLabel = { _, context ->
                val label = context.getString(R.string.quiver_guard_exception_state_label)
    ...
    // Reads the current exception list from SitePermissionDatabase and pushes it
    // to the adapter. Also updates the empty-state view and fast scroller.
    private fun refreshList() {
        val items = SitePermissionManager.getAllByType(this, SitePermissionDatabase.TYPE_QUIVER_GUARD_EXCEPTION)
        adapter.updateItems(items)
        updateEmptyState()
        fastScroller.notifyDataChanged()
    }

    // Shows an empty-state label when the list contains no excepted sites, and
    // hides the RecyclerView and fast scroller so the layout is clean.
    ...
    // Updates the toolbar title, back-button icon, and FAB visibility to reflect
    ...
    private fun updateSelectionUi(selectedCount: Int) {
        ...
        if (inSelectionMode) {
            ...
            toolbarTitle.text = getString(R.string.site_settings_quiver_guard)
    ...
    // Deletes the selected exception entries from the database, removes them from
    // the adapter, and shows a toast with the deletion count.
    private fun showDeleteConfirmDialog() {
        ...
        MaterialAlertDialogBuilder(this, getDialogTheme())
            .setTitle(getString(R.string.quiver_guard_exception_delete_title))
            .setMessage(getString(R.string.quiver_guard_exception_delete_message, count))
            .setNegativeButton(android.R.string.cancel, null)
            .setPositiveButton(getString(R.string.history_delete_selected)) { _, _ ->
                val origins = adapter.getSelectedOrigins()
                for (origin in origins) {
                    SitePermissionManager.deleteEntry(this, origin, SitePermissionDatabase.TYPE_QUIVER_GUARD_EXCEPTION)
                }
                adapter.removeSelectedItems()
                updateSelectionUi(0)
                fastScroller.notifyDataChanged()
                ClintToast.show(this, getString(R.string.quiver_guard_exception_items_deleted), R.drawable.ic_delete_24)
            }
            .create().also { applyStatusBarFlagToDialog(it) }.show()
    }

    // Shows a simple text field dialog that accepts a hostname or URL. The input
    // is normalised to the registered domain (eTLD+1) before being stored so that
    // "https://www.example.com/path" and "example.com" both produce "example.com".
    private fun showAddSiteDialog() {
        ...
        // The allowed/denied radio buttons are not relevant for exceptions; hide them.
        ...
        MaterialAlertDialogBuilder(this, getDialogTheme())
            .setTitle(getString(R.string.quiver_guard_exception_add_site))
            .setView(dialogView)
            .setNegativeButton(android.R.string.cancel, null)
            .setPositiveButton(getString(R.string.site_permission_add)) { _, _ ->
                ...
                if (origin.isNotEmpty()) {
                    SitePermissionManager.setState(
                        this, origin,
                        SitePermissionDatabase.TYPE_QUIVER_GUARD_EXCEPTION,
    ...
    // Extracts the registered domain (eTLD+1) from user input which may be a bare
    // hostname, a URL with a scheme, or a path. Prepends https:// if no scheme is
    // present so Uri.parse produces a valid authority component.
