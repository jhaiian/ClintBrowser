...
[adblock-rust](https://github.com/brave/adblock-rust), pinned to `=0.13.3`.
...
The `adblock` crate is pinned to exactly `0.13.3` rather than using a caret
...
### Upstream changelog since the last pin (0.13.2 -> 0.13.3)

From upstream's `CHANGELOG.md`:

- **0.13.3** - Added: preliminary parsing of `$to`, not yet used by matching
  logic. Nothing to do here.
- **0.13.3** - Changed: DAT format version bumped to v6. Engine files compiled
  by the previous pin now fail `deserialize()` with
  `DeserializationError::VersionMismatch`, which `nativeLoadEngine` already
  maps to its `-1` sentinel so Kotlin recompiles from the source filter lists -
  no source change needed, but every on-device cached engine gets rebuilt once
  after this update ships.
- **0.13.3** - Fixed: regex domains can no longer produce invalid content
  blocking rules. Internal validation fix, no signature change.

Net result: no changes were required in `src/lib.rs` for this bump, since none
of the touched APIs are used by this crate. `Cargo.lock` is regenerated by the
**Build native library** workflow (`cargo check` / `cargo ndk build`), not
committed by hand - see "Building" above.
